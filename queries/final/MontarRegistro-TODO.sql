...
LEFT JOIN (
	SELECT cod_acervo, STRING_AGG(CAST(CONCAT(cod_item, '$a',sigla_unidade,'$b',sigla_unidade, '$d', CONVERT(varchar,dt_registro,23),'$h','v. ',TRIM(ISNULL(volume,'')),', t. ',tomo,'$i',patrimonio,cod_old,'$o',classificacao,'$t',TRIM(exemp),'$u',anexos2.anexos,'$y','$z',TRIM(str_tp_aqui),'$0',baixado, '$7', REPLACE(REPLACE(REPLACE(empresta,'0','2'),'1','0'),'2','1'))AS VARCHAR(MAX)), ';-;') as items
